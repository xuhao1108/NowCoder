...
class ChromeDriver(object):
    def __init__(self, executable_path='chromedriver', binary_location=None, wait_time=30, implicitly_wait_time=0, headless=False,
                 ...
                 chrome_data_dir=None, proxy_info=None, auth_proxy_info=None, phone_info=None,
        ...
        if debugger_address_info:
            ...
            if chrome_data_dir:
                ...
                options.add_argument(r'--user-data-dir={}'.format(chrome_data_dir))
            # 代理
            if proxy_info:
        ...
        # self.chrome.implicitly_wait(implicitly_wait_time)
    ...
    def click_element_by_action(self, element):
        """
        点击元素
        :param element: 元素
        :return:
    ...
    def reset_acitons(self):
        ...
        :return:
        """
        ActionChains(self.chrome).reset_actions()
    ...
    def click_and_hode_element_by_action(self, element, wait_time=0.1):
        """
        点击元素
        :param element: 元素
        ...
        :return:
        """
        ActionChains(self.chrome).click_and_hold(element).perform()
        time.sleep(wait_time)
        ActionChains(self.chrome).release(element).perform()
    ...
    def slider_element(self, element, distance, offset=[10, -5], add_a=2, reduce_a=-3, t=0.2, v=0, mid=None, wait_time=0.5):
        """
        滑动元素，一定的距离
        :param element: 滑动的元素
        :param distance: 总路程
        ...
        ActionChains(self.chrome).click_and_hold(element).perform()
        # 拖动“滑块”
        for x in self.get_track(distance, add_a, reduce_a, t, v, mid):
            ActionChains(self.chrome).move_by_offset(xoffset=x, yoffset=0).perform()
        # 偏移量
        for x in offset:
            ActionChains(self.chrome).move_by_offset(xoffset=x, yoffset=0).perform()
        time.sleep(wait_time)
    ...
    def move_element(self, element, x, y=0, wait_time=0.5):
        # 点击“滑块”
        ActionChains(self.chrome).click_and_hold(element).perform()
        # 拖动“滑块”
        ActionChains(self.chrome).move_by_offset(xoffset=x, yoffset=y).perform()
        time.sleep(wait_time)
        # 松开“滑块”
        ActionChains(self.chrome).release().perform()
